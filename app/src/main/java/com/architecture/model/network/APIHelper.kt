...
object APIHelper {
    ...
    fun signUp(email: String, pwd: String):  LiveData<Result<SignupRes>> {

        var signupRes: MutableLiveData<Result<SignupRes>> = MutableLiveData()
        val request = SignupReq()
        request.email = email
        request.password = pwd

       val call= apiClient.signup(request)

        call.enqueue(object : Callback<SignupRes>{

            override fun onFailure(call: Call<SignupRes>, t: Throwable) {
                val exception = Exception("Something went wrong.")
                signupRes.value = Result.failure(exception)
            }

            override fun onResponse(call: Call<SignupRes>, response: Response<SignupRes>) {
                if(response.isSuccessful) {
                    signupRes.value = Result.success(response.body()!!)
                } else if(response.code() == 400){
                    var message = "Something went wrong."
                    try {
                        val jObjError = JSONObject(response.errorBody()!!.string())
